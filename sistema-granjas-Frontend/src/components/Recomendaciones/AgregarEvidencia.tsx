// src/components/Recomendaciones/AgregarEvidenciaModal.tsx
...
const AgregarEvidenciaModal: React.FC<AgregarEvidenciaModalProps> = ({
    ...
    // Los hooks deben estar al inicio, antes de cualquier condicional
    const [file, setFile] = useState<File | null>(null);
    const [descripcion, setDescripcion] = useState('');
    const [tipo, setTipo] = useState('imagen');
    const [uploading, setUploading] = useState(false);

    // Si no hay recomendación seleccionada, no renderizar el modal
                ...
                <div className="flex justify-between items-center mb-6 pb-4 border-b">
                    <div>
                        <h2 className="text-xl font-bold text-gray-800">
                            Agregar Evidencia a Recomendación
                        </h2>
                        <p className="text-sm text-gray-600 mt-1">
                            Recomendación: <span className="font-medium">{recomendacion.titulo}</span> (ID: #{recomendacion.id})
                        </p>
                    </div>
                </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                rows={3}
                                placeholder="Describe esta evidencia. Ej: 'Fotografía del sistema de riego', 'Documento técnico sobre fertilización', etc."
                                required
                            />
                        </div>
